...
FROM
  ...
  WITH seq AS
    ...
    FROM VME.SPECIFIC_MEASURE 
    GROUP BY VME_ID) lastmeasure
    WHERE END_YEAR IS NULL 
    AND seq.VME_ID = lastmeasure.VME_ID
  ) geotime,
...
where vme_id = 'VME_NAFO_6'


--select * from vme.georef
--where geographicfeatureid like 'VME_NAFO_6%'
