...
public class CRUDJpaDaoTest {
	...
	public void testDeleteVme() {
		...
		for (SpecificMeasure measure : specificMeasures) {
			...
			if (measure.getInformationSource() != null) {
				System.out.println("Checking non-nullity of previously linked information source with ID "
						+ measure.getInformationSource().getId());
				Assert.assertNotNull(this.vmeDao.getEntityById(this.em, InformationSource.class, measure
	...
	public void testDeleteGeoRefFromVme() {
		...
		Assert.assertTrue(vme.getGeoRefList() == null || vme.getGeoRefList().isEmpty()
				|| (vme.getGeoRefList().size() == 1 && vme.getGeoRefList().get(0) == null));
	...
	public void testDeleteInformationSource() {
		...
		if (!informationSource.getSpecificMeasureList().isEmpty()) {
			...
			for (SpecificMeasure specificMeasure : informationSource.getSpecificMeasureList()) {
				Assert.assertTrue(this.vmeDao.getEntityById(this.vmeDao.getEm(), SpecificMeasure.class,
						specificMeasure.getId()).getInformationSource() == null);
			}
		}
		...
		if (!informationSource.getGeneralMeasureList().isEmpty()) {
			for (GeneralMeasure generalMeasure : informationSource.getGeneralMeasureList()) {
				Iterator<InformationSource> generalMeasureIterator = generalMeasure.getInformationSourceList()
						.iterator();
		...
		informationSource = this.vmeDao.getEntityById(this.vmeDao.getEm(), InformationSource.class,
				informationSource.getId());

		assertFals(vmeDao.getEm().contains(informationSource));

		// Assert.assertNull(informationSource);
	}

	private void assertFals(boolean contains) {
