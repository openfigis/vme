...
public class ReportManagerReportBuilderTest extends AbstractCompilerDependentTest {
	...
	@Inject
	@Builder
	private ReportBuilder<Model> _reportBuilder;
	@Inject
	@Evaluator
	...
	public void testBuildAndExtractVmeReport() throws Throwable {
		...
		report = this._reportEvaluator.evaluate(template,
				this._vmeDao.getEntityById(this._vmeDao.getEm(), Vme.class, new Long(2)));

		Model model = this._reportBuilder.buildReport(report);

		// this.dumpModel(model);

		// System.out.println("Scanning model instance with id: " +
		// model.getUniqueID() + ", name: " + model.getTemplateName());
